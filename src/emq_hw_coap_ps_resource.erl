...
-module(emq_hw_coap_ps_resource).
...
coap_observe(ChId, ?PS_PREFIX, [Topic], Ack, Content) ->
    ...
    Code = case emq_hw_coap_ps_topics:is_topic_timeout(TrueTopic) of
               true  ->
                   nocontent;
               false->
                   content
...
handle_info({dispatch, Topic, Payload}, State) ->
    ...
    {ok, Ret} = emq_hw_coap_ps_topics:reset_topic_info(Topic, Payload),
...
add_topic_info(publish, Topic, MaxAge, Format, Payload) when is_binary(Topic), Topic =/= <<>>  ->
    case emq_hw_coap_ps_topics:lookup_topic_info(Topic) of
                ...
                true  ->
                    {ok, Ret} =
                        case StoredMaxAge =:= MaxAge of
                            true  ->
                                emq_hw_coap_ps_topics:reset_topic_info(Topic, Payload);
                            false ->
                                emq_hw_coap_ps_topics:reset_topic_info(Topic, MaxAge, Payload)
                        end,
                    {changed, Ret};
                false ->
        ...
        [] ->
            ...
            {ok, Ret} = emq_hw_coap_ps_topics:add_topic_info(Topic, MaxAge, Format, Payload),
            {created, Ret}
    end;

add_topic_info(create, Topic, MaxAge, Format, _Payload) when is_binary(Topic), Topic =/= <<>> ->
    case emq_hw_coap_ps_topics:is_topic_existed(Topic) of
        true ->
            ...
            {ok, Ret} = emq_hw_coap_ps_topics:reset_topic_info(Topic, MaxAge, Format, <<>>);
        false ->
            ?LOG(debug, "create topic=~p will be created", [Topic]),
            {ok, Ret} = emq_hw_coap_ps_topics:add_topic_info(Topic, MaxAge, Format, <<>>)
    end,
    {created, Ret};
...
delete_topic_info(Topic) ->
    case emq_hw_coap_ps_topics:lookup_topic_info(Topic) of
        [] ->
            {error, not_found};
        [{_, _, _, _, _}] ->
            emq_hw_coap_ps_topics:delete_sub_topics(Topic)
    end.
